...
class GpuPartitioningSuite extends AnyFunSuite {
  ...
  private def testGpuPartitionWithCompression(codecName: String,
      serdeType: String = "CPU"): Unit = {
    val conf = new SparkConf()
  ...
  test("GPU partition with GPU Serde") {
  ...
  test("GPU partition with lz4 compression with GPU Serde") {
    testGpuPartitionWithCompression("lz4", "GPU")
  }

  test("GPU partition with zstd compression with GPU Serde") {
    testGpuPartitionWithCompression("zstd", "GPU")
  }
}
